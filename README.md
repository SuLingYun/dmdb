...
```
dmdb-recovery/
├── dm_recover.sh           # 主恢复脚本（支持五种模式）
├── reset_dm.sh             # 重新初始化脚本（重置实例 + 归档 + 备份作业）
├── rsync-inotify-sync.sh   # rsync+inotify 实时文件同步工具（双模式）
├── inotify-tools-3.22.1.0.tar.gz  # inotify-tools 源码包
├── README.md               # 项目说明文档
├── RESET_GUIDE.md          # reset_dm.sh 使用指南
├── RSYNC_SYNC.md           # rsync-inotify-sync.sh 使用指南
...
```bash
# 定期清理旧日志（保留30天）
find /data/dmbak/DAMENG -name "recover_*.log" -mtime +30 -delete
```
...
- 新增 `RSYNC_SYNC.md` 使用指南
- 新增 inotify-tools 源码包
- 完善文档结构，精简冗余文档

### v1.1.0 (2026-06-15)

- 新增 `reset_dm.sh` 数据库重新初始化脚本
- 新增 `RESET_GUIDE.md` 使用指南
- 支持 dmrman 脱机备份（模式4）和 disql 联机备份（模式5）
- 模式3支持增量选择（全部/部分/不使用）

### v1.0.0 (2026-06-11)

- 初始版本发布
- 支持三种恢复模式（恢复到最新/时间点/仅恢复备份）
- 实现可视化进度条
